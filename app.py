...
def load_models():
    try:
        model_dir = os.path.join(os.getcwd(), "models")  # Get absolute path
        ...
        print(f"❌ Error loading models: {e}")
        raise  # Stop execution if models aren't loaded
...
def index():
    if request.method == 'POST':
        try:
            ...
            return render_template('index.html', error=f"⚠️ Error: {str(e)}")
